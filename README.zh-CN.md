...
ClawPlus 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
ClawPlus 支持**便携模式**，所有应用数据（配置、AI 供应商密钥、会话记录、Python 环境）与可执行文件保存在同一驱动器上——非常适合 U 盘携带或无安装权限的受限机器。

### 工作原理

应用根目录下的 `.portable` 标记文件会在启动时激活便携模式。所有用户数据存储在可执行文件旁的 `LocalData/` 文件夹中，而非系统默认目录（`%APPDATA%` 等）。

支持**两种便携安装类型**，由 `.portable` 文件首行内容决定：

| 类型   | 标记内容 | 说明                                             |
| ------ | -------- | ------------------------------------------------ |
| `dir`  | `dir`    | 解压目录版本。通过下载 zip + robocopy 方式更新。 |
| `nsis` | `nsis`   | NSIS 安装包带便携标记。使用标准 NSIS 更新流程。  |
...
生成 `win-unpacked/` 文件夹，包含 `.portable` 标记文件（类型 `dir`）、`LocalData/` 数据目录和 `README-portable.txt` 说明。
...
生成标准 `.exe` 安装包，安装时检测 `.portable` 标记并将数据保留在本地。

### 便携版目录结构

```
H:\ClawPlus\                      # U 盘根目录（示例）
├── ClawPlus.exe                  # 主程序
├── .portable                     # 标记文件（首行：dir 或 nsis）
├── LocalData/                    # 所有用户数据存放于此
│   ├── config/                   # electron-store JSON 配置
│   ├── openclaw/                 # OpenClaw 运行时数据
│   ├── python/                   # 内置 Python（通过 uv 自动下载）
│   ├── logs/                     # 应用日志
│   └── cache/                    # 临时与缓存数据
├── resources/                    # 应用资源
└── ...                           # 其他 Electron 文件
...
- **dir 便携版**：更新器从 CDN 下载 `.zip` 文件，解压到暂存区域，下次重启时通过 `robocopy` 应用更新——保留 `LocalData/` 和 `.portable` 标记文件。
- **nsis 便携版**：使用标准 electron-updater NSIS 更新流程；安装脚本检测到 `.portable` 后跳过注册表和 PATH 操作。

> **注意**：便携模式目前仅支持 Windows 平台。
...
# 便携版打包（仅 Windows）
pnpm run package:portable         # 目录便携版（解压文件夹）
pnpm run package:portable:nsis    # NSIS 便携安装包
```
