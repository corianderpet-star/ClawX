...
/**
 * Merge a ClawPlus context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeClawPlusSection(existing: string, section: string): string {
...
/**
 * Detect and remove bootstrap .md files that contain only ClawPlus markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairClawPlusOnlyBootstrapFiles(): Promise<void> {
    ...
    for (const file of entries) {
      ...
      if (before === '' && after === '') {
        try {
          ...
          logger.info(`Removed ClawPlus-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove ClawPlus-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge ClawPlus context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeClawPlusContextOnce(): Promise<number> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('ClawPlus context directory not found, skipping context merge');
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      ...
      const merged = mergeClawPlusSection(existing, section);
      if (merged !== existing) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged ClawPlus context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure ClawPlus context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureClawPlusContext(): Promise<void> {
  let skipped = await mergeClawPlusContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeClawPlusContextOnce();
    if (skipped === 0) {
      logger.info(`ClawPlus context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`ClawPlus context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`ClawPlus context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
