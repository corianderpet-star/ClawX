...
/**
 * Write the ClawPlus gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  ...
  // Packaged ClawPlus loads the renderer from file://, so the gateway must allow
 ...
 * OpenClaw/ClawPlus versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  ...
  // When ClawPlus-managed moonshot provider exists, prefer centralized key
