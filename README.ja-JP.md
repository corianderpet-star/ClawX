...
ClawPlusは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
ClawPlusは**ポータブルモード**をサポートしており、すべてのアプリケーションデータ（設定、AIプロバイダーキー、セッション履歴、Python環境）を実行ファイルと同じドライブに保存します。USBドライブやインストール権限のない環境に最適です。

### 仕組み

アプリケーションルートにある `.portable` マーカーファイルが、起動時にポータブルモードを有効にします。すべてのユーザーデータは、OSのデフォルトディレクトリ（`%APPDATA%` など）ではなく、実行ファイルの隣にある `LocalData/` フォルダに保存されます。

`.portable` ファイルの1行目で識別される**2種類のポータブルインストールタイプ**があります：

| タイプ | マーカー内容 | 説明                                                           |
| ------ | ------------ | -------------------------------------------------------------- |
| `dir`  | `dir`        | 展開済みディレクトリビルド。zipダウンロード + robocopyで更新。 |
| `nsis` | `nsis`       | ポータブルフラグ付きNSISインストーラー。標準NSIS更新を使用。   |
...
`.portable`（タイプ `dir`）、`LocalData/` ディレクトリ、`README-portable.txt` を含む `win-unpacked/` フォルダを生成します。
...
インストール時に `.portable` マーカーを検出し、データをローカルに保持する標準 `.exe` インストーラーを生成します。

### ポータブル版ディレクトリ構成

```
H:\ClawPlus\                      # USBルート（例）
├── ClawPlus.exe                  # メイン実行ファイル
├── .portable                     # マーカーファイル（1行目: dir or nsis）
├── LocalData/                    # すべてのユーザーデータはここに保存
│   ├── config/                   # electron-store JSON設定
│   ├── openclaw/                 # OpenClawランタイムデータ
│   ├── python/                   # バンドルPython（uvで自動ダウンロード）
│   ├── logs/                     # アプリケーションログ
│   └── cache/                    # 一時・キャッシュデータ
├── resources/                    # アプリリソース
└── ...                           # その他のElectronファイル
...
- **dirポータブル**: アップデーターがCDNから `.zip` をダウンロードし、ステージングエリアに展開後、次回再起動時に `robocopy` で適用します。`LocalData/` と `.portable` マーカーは保持されます。
- **nsisポータブル**: 標準のelectron-updater NSISフローを使用。インストーラースクリプトが `.portable` を検出し、レジストリ/PATH操作をスキップします。

> **注意**: ポータブルモードは現在Windowsのみ対応しています。
...
# ポータブルビルド（Windowsのみ）
pnpm run package:portable         # ディレクトリポータブル（展開フォルダ）
pnpm run package:portable:nsis    # NSISポータブルインストーラー
```
