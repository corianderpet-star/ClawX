...
複数のAIエージェントを作成・設定・切り替えできます。各エージェントは独自のモデル、スキル、SOUL.md パーソナリティ定義を持つことができます。リーダー/サブの役割による**企業スタイルの階層構造**をサポートし、インタラクティブな**組織図**で可視化します。専用の**エージェント設定**ダイアログで名前やチャンネルバインドを管理でき、特定のエージェントをチャンネルや定期タスクに割り当てて精密な制御が可能です。
