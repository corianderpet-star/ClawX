...
# Post-installation script for ClawPlus on Linux
...
# Create symbolic link for ClawPlus app binary
if [ -x /opt/ClawPlus/clawx ]; then
    ln -sf /opt/ClawPlus/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/ClawPlus/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/ClawPlus/chrome-sandbox' || true
else
    chmod 0755 '/opt/ClawPlus/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/ClawPlus/resources/apparmor-profile'
...
echo "ClawPlus has been installed successfully."
